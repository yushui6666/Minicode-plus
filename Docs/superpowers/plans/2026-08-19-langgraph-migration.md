...
### Task 7: Graph parity — store/provider fallback, callbacks, and default flip (slice 5)
...
- [x] Wire `store` + streaming kwargs into `next_step` (inspect `model.next` signature, forward `store`/`on_thinking_delta`/`on_stream_chunk`), and surface provider-switch logic (at least: typed error → fallback chain attempt before emitting `kind="error"` blocked stop; mirror `agent_loop._model_next` + fallback guidance). Streaming flags via `callbacks.include_stream_chunks/include_thinking_chunks` → `on_stream_chunk`/`on_thinking_delta`; fallback retry via `ModelSwitcher` (1 retry, emits `recovery` RuntimeEvent, augments `fallback_errors` with `no viable fallback…` + guidance via `describe_fallback_guidance`/`describe_provider_channel`).
- [x] Align callbacks: `assistant_followup`/`step_policy` progress events vs legacy `contentKind="progress"` nudges; ensure `on_tool_start`/`on_tool_result`/`on_assistant_message`/`on_progress_message`/`on_runtime_event` parity via `TurnEventQueue` adapters — `_model_step_kernel` now takes `sink` and `sink.progress` for `tool_calls` with `contentKind=progress`; `assistant_followup`/`step_policy` emit `sink.progress` for `recovery`/`guard`/`phase`; structured `TurnEvent` publish via `callbacks.on_event` (fallback to named handlers) + `TurnEvent.completed` (done) after `graph.invoke`.
- [x] Flip shim default: `want_graph` defaults True; rollback via `MINICODE_USE_GRAPH=0` / `runtime={"useGraph": False}` (env `"0"/"false"/"no"/"off"` disables, runtime `"useGraph"` key overrides). Legacy loop remains as fallback on graph exception (`RuntimeWarning`).
- [x] Verify: `MINICODE_USE_GRAPH=1` and default (now graph) both 24 passed in `tests/test_agent_loop.py`; `tests/test_turn_events.py` + `tests/test_langgraph_runtime.py` + `tests/test_graph_multi_tool.py` + `tests/test_graph_checkpoint.py` 42 passed; focused regression (headless/tty/turn_kernel/agent_flow/tools/permissions/session/mcp/hooks/compactor/memory/skills) 340 passed; full suite 1424 passed, 2 skipped (1 known `test_provider_smoke_reaches_local_openai...` PermissionError sandbox noise, excluded via `--ignore`). `MINICODE_USE_GRAPH=0` still routes to legacy loop (24 passed).

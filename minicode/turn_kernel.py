...
#
# The LangGraph runtime (minicode/graph) keeps turn state as plain
# serializable fields inside AgentState, per the migration spec's
# "nodes communicate through plain serializable values" boundary.  These
# helpers convert between that flat snapshot and TurnRecurrentState so
# graph nodes can reuse the kernel decision functions above (including
# their documented counter side effects) without duplicating semantics.
# Round-trip rule: snapshot_to_turn_state(turn_state_to_snapshot(ts))
# must reproduce every field the kernel functions read.


def snapshot_to_turn_state(snapshot: Mapping[str, Any]) -> TurnRecurrentState:
    """Rebuild a TurnRecurrentState from flat graph-state fields."""

    turn_state = TurnRecurrentState(
        max_steps=snapshot.get("max_steps"),
        profile_name=str(snapshot.get("profile_name") or "single"),
        widen_after_step=snapshot.get("widen_after_step"),
        empty_response_retry_limit=int(snapshot.get("empty_response_retry_limit", 2)),
        recoverable_thinking_retry_limit=int(
            snapshot.get("recoverable_thinking_retry_limit", 3)
        ),
        saw_tool_result=bool(snapshot.get("saw_tool_result", False)),
        empty_response_retry_count=int(snapshot.get("empty_response_retry_count", 0)),
        recoverable_thinking_retry_count=int(
            snapshot.get("recoverable_thinking_retry_count", 0)
        ),
        tool_error_count=int(snapshot.get("tool_error_count", 0)),
        tool_observation_count=int(snapshot.get("tool_observation_count", 0)),
        successful_tool_observation_count=int(
            snapshot.get("successful_tool_observation_count", 0)
        ),
        step=int(snapshot.get("step", 0)),
        widening_active=bool(snapshot.get("widening_active", False)),
        widening_transition_count=int(snapshot.get("widening_transition_count", 0)),
        widening_trigger_reason=str(snapshot.get("widening_trigger_reason", "") or ""),
        widening_trigger_evidence=str(
            snapshot.get("widening_trigger_evidence", "") or ""
        ),
        latest_tool_result_summary=str(
        ...
        verification_state=TurnVerificationState(
            ...
            requires_explicit_final=bool(
                snapshot.get("verification_requires_explicit_final", False)
            ),
            requires_evidence=bool(snapshot.get("verification_requires_evidence", False)),
            evidence_ready=bool(snapshot.get("verification_evidence_ready", False)),
            evidence_summary=str(snapshot.get("verification_evidence_summary", "") or ""),
            last_verification_note=str(
                snapshot.get("verification_last_note", "") or ""
            ),
        ),
        step_policy=TurnStepPolicy(
            phase=snapshot.get("policy_phase", "explore"),
            phase_index=int(snapshot.get("policy_phase_index", 0)),
            remaining_steps=snapshot.get("policy_remaining_steps"),
            guidance=str(snapshot.get("policy_guidance", "") or ""),
            verification_focus=str(
                snapshot.get("policy_verification_focus", "light") or "light"
            ),
            allow_widening=bool(snapshot.get("policy_allow_widening", False)),
            widening_active=bool(snapshot.get("widening_active", False)),
            widening_reason=str(snapshot.get("widening_trigger_reason", "") or ""),
            widening_evidence_summary=str(
                snapshot.get("widening_trigger_evidence", "") or ""
            ),
            should_compact_aggressively=bool(
                snapshot.get("policy_should_compact_aggressively", False)
            ),
        ),
    )
...
def turn_state_to_snapshot(turn_state: TurnRecurrentState) -> dict[str, Any]:
    """Flatten a TurnRecurrentState into plain graph-state fields."""

    policy = turn_state.step_policy
    verification = turn_state.verification_state
    return {
        "step": turn_state.step,
        "max_steps": turn_state.max_steps,
        "profile_name": turn_state.profile_name,
        "widen_after_step": turn_state.widen_after_step,
        "empty_response_retry_limit": turn_state.empty_response_retry_limit,
        "empty_response_retry_count": turn_state.empty_response_retry_count,
        "recoverable_thinking_retry_limit": turn_state.recoverable_thinking_retry_limit,
        "recoverable_thinking_retry_count": turn_state.recoverable_thinking_retry_count,
        "saw_tool_result": turn_state.saw_tool_result,
        "tool_error_count": turn_state.tool_error_count,
        "tool_observation_count": turn_state.tool_observation_count,
        "successful_tool_observation_count": (
            turn_state.successful_tool_observation_count
        ),
        "widening_active": turn_state.widening_active,
        "widening_transition_count": turn_state.widening_transition_count,
        ...
        "verification_requires_evidence": verification.requires_evidence,
        "verification_evidence_ready": verification.evidence_ready,
        "verification_evidence_summary": verification.evidence_summary,
        "verification_last_note": verification.last_verification_note,
        "policy_phase": policy.phase,
        "policy_phase_index": policy.phase_index,
        "policy_remaining_steps": policy.remaining_steps,
        "policy_guidance": policy.guidance,
        "policy_verification_focus": policy.verification_focus,
        "policy_allow_widening": policy.allow_widening,
        "policy_should_compact_aggressively": policy.should_compact_aggressively,
    }
